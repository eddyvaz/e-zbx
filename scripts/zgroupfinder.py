...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,description='Prints Groups Zabbix.', epilog="""
...
# list groups
try:
    for groups in zapi.hostgroup.get({"output": "extend", "sortfield": "name"}):
       print(format(groups["name"]))
except:
    error="Error: Could not retrieve groups Zabbix"
    PrintError(error)
